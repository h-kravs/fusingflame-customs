...
import { IconPrefix, IconName, IconDefinition, findIconDefinition, library, SizeProp } from '@fortawesome/fontawesome-svg-core'
import { fas } from '@fortawesome/free-solid-svg-icons'

library.add(fas)

const Tire = () => (
  <svg className="card-icon" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
...
export const Icon = (iconName: IconName | string, size?: SizeProp, beat?: boolean): ReactNode => {
  // Check if it's a PNG file
  if (typeof iconName === 'string' && iconName.endsWith('.png')) {
    try {
      // Try to import the icon from the icons directory
      const iconModule = require(`../icons/${iconName}`);
      const iconPath = iconModule.default || iconModule;
      return (
        <img 
          src={iconPath} 
          alt={iconName} 
      ...
      // console.warn(`PNG icon not found: ${iconName}`, error);
      ...
      return (
        <img 
          ...
          style={{ width: '40px', height: '40px', objectFit: 'contain' }}
          onError={(e) => {
            console.warn(`PNG icon failed to load: ${iconName}`);
            // Replace with placeholder div
            const placeholder = document.createElement('div');
            placeholder.className = 'card-icon';
            placeholder.style.cssText = 'width: 40px; height: 40px; background-color: #666; display: flex; align-items: center; justify-content: center; color: white; font-size: 10px;';
  ...
  const iconDefinition: IconDefinition = findIconDefinition({ prefix: 'fas' as IconPrefix, iconName: iconName as IconName });
  if (!iconDefinition) {
    if (iconName === 'tire') return <Tire/>
    // If no FontAwesome icon found, return a placeholder
    return <div className='card-icon' style={{ width: '40px', height: '40px', backgroundColor: '#ccc' }}></div>;
  }
  return <FontAwesomeIcon icon={iconDefinition} size={size || "2x"} pulse={beat} className='card-icon' />;
};
